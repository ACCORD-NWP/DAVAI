...
class OOPSAnalysisLAM3DEnVar(Task, DavaiIALTaskMixin, IncludesTaskMixin):
    ...
    def process(self):
        ...
        if 'early-fetch' in self.steps or 'fetch' in self.steps:
            self._wrapped_input(
                ...
                nativefmt      = 'json',
                objects        = 'analyse-3DEnVarSDL_aro',
                scope          = 'oops',
                path           = f'config_oops/{self.conf.suite_vapp}/ae/[objects].[nativefmt]',
                ref            = self.conf.gitenv_ref,
                repo           = self.conf.gitenv_repo,
            )
            #-------------------------------------------------------------------------------
