    ...
    def process(self):
        ...
        if 'early-fetch' in self.steps or 'fetch' in self.steps:
            ...
            if self.conf.geonam_source == 'static':
                tbgeo = self._wrapped_input(  # this is stored in shelf because generated by domain_maker
                ...
                self._wrapped_input(
                    ...
                    path           = f'namelist/{self.conf.suite_app}/{self.conf.pgd_suite_conf}/namel_buildpgd',
        ...
        if 'fetch' in self.steps:
            if self.conf.geonam_source == 'flow':
                ...
                #-------------------------------------------------------------------------------
                # this is actually static but needs to be updated with geometry, fetched right above
                self._wrapped_input(
                    role           = 'Namelist',
                    hook_geo       = (update_namelist, tbgeo),
                    intent         = 'inout',
                    kind           = 'namelist',
                    local          = 'OPTIONS.nam',
                    path           = f'namelist/{self.conf.suite_app}/{self.conf.suite_conf}/namel_buildpgd',
                    ref            = self.conf.gitenv_ref,
                    repo           = self.conf.gitenv_repo,
                )
                #-------------------------------------------------------------------------------
