# -*- coding: utf-8 -*-
"""
4DVar task with hybrid 3D covariances.

Two inner loops are run at the same resolution TL63c1 (global63).
The outer loop trajectory are run at higher resolution, TL149c1 (globalupd149)

The second loop minimization is preconditioned with spectral information from
the first loop.

A simplified static component is used for B: spectral-based correlations rather
than spectral correlations, uniform sigma_b rather than flow-dependent variances.

The ensemble B is based on 8 ensemble members generated by randomization.
"""
...
class Analysis4dvar(Task, DavaiIALTaskMixin, IncludesTaskMixin):
    ...
    def process(self):
        ...
        if 'early-fetch' in self.steps or 'fetch' in self.steps:
            ...
            # It must be read during execution, but the content is not actually used
            # since the simple spectral B used here uses constant variances.
            self._wrapped_input(
                role           = 'BackgroundStdError',
                block          = 'sigmab',
                date           = '{}/-{}'.format(self.conf.rundate.ymdh, self.conf.cyclestep),
            ...
            self._wrapped_input(
                ...
                intent         = 'in',
                kind           = 'historic',
                local          = 'ens_xb_[geometry::tag]_m[member]_d[date:ymdh]_t[term:fmthm]',
